...
def add(nums: list[int], target: int) -> list[int]:
    ...
    if any(type(el)!=int for el in nums):
        ...
        raise NumsContainsNotIntError(f"list[int] argument contains non-int elements (\n"+s+"\n)")
    
    if type(nums) != list:
        raise NumsNotListError("list[int] isn't instance of List()")
    
    if type(target) != int:
        raise TargetIsNANError("target isn't instance of int()")
